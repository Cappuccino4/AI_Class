...
two_ways_x(S) :- valid_state(S), one_way_x(S, A), one_way_x(S, B), \+ A=B, print_state(S).

one_way_x(S, A) :- seq(S, [X,Y,Z], [A,_,_]), (X=b, Y=x, Z=x).
one_way_x(S, A) :- seq(S, [X,Y,Z], [_,A,_]), (X=x, Y=b, Z=x).
one_way_x(S, A) :- seq(S, [X,Y,Z], [_,_,A]), (X=x, Y=x, Z=b).

no_ways_x(S) :- valid_state(S), not(one_way_x(S, _)), print_state(S).